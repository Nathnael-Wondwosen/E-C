...
const getUserName = () => {
  if (typeof window !== 'undefined') {
    ...
    // For now, return email as name or a default value
    // In a real implementation, you might store the name separately
    return userEmail ? userEmail.split('@')[0] : 'User';
  }
  return 'User';
};

// Helper function to protect routes that require login
const protectRoute = (route) => {
  if (!isLoggedIn()) {
    ...
    // For logged-in users, navigate to the route directly
    window.location.href = route;
  }
};

// Helper function to navigate to routes without protection (for logged-in users)
...
export default function Header({ isMenuOpen, setIsMenuOpen, categories }) {
  ...
  useEffect(() => {
    ...
    // Load cart and wishlist counts if user is logged in
    if (isLoggedIn()) {
    ...
    const handleStorageChange = () => {
      ...
      // Update counts when login status changes
      if (loggedIn) {
        loadCartAndWishlistCounts();
      } else {
        setCartCount(0);
  ...
  // Force refresh of login status when component updates
  useEffect(() => {
    const refreshLoginStatus = () => {
      setIsUserLoggedIn(isLoggedIn());
      setUserType(getUserType());
      
      // Refresh counts when login status changes
      if (isLoggedIn()) {
        loadCartAndWishlistCounts();
      }
    };
    
    // Add a custom event listener for login/logout events
  ...
  // Function to load cart and wishlist counts
  ...
  // Function to update login status externally
      ...
      <header className="bg-gradient-to-r from-gray-800 to-gray-900 shadow-lg sticky top-0 z-50">
        <div className="container mx-auto px-4">
          ...
          <div className="hidden md:flex items-center justify-between py-4">
            ...
            <div className="flex items-center space-x-6">
              <Link href="#" onClick={() => isUserLoggedIn ? navigateToRoute('/wishlist') : protectRoute('/wishlist')} className="flex flex-col items-center text-gray-300 hover:text-white transition-colors duration-300">
                <div className="relative">
              ...
              <Link href="#" onClick={() => isUserLoggedIn ? navigateToRoute('/cart') : protectRoute('/cart')} className="flex flex-col items-center text-gray-300 hover:text-white transition-colors duration-300">
                <div className="relative">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              {/* Account Link - Changes based on login status */}
              <div className="flex flex-col items-center">
                {isUserLoggedIn ? (
                  <div className="relative group">
                    {/* Account icon that only shows dropdown on hover/click */}
                    <button 
                      type="button"
                      className="flex flex-col items-center text-gray-300 hover:text-white transition-colors duration-300 cursor-pointer bg-transparent border-none p-0"
                    >
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="absolute right-0 top-full mt-2 w-56 bg-gradient-to-b from-gray-800 to-gray-900 text-white shadow-2xl rounded-none border border-gray-700 hidden group-hover:block z-50">
                      <div className="py-2">
                        {/* User name section */}
                        <div className="px-4 py-3 border-b border-gray-700 bg-gray-800 bg-opacity-50">
                          <p className="font-medium text-amber-400">Welcome, {getUserName()}!</p>
                        </div>
                        
                        {/* Dashboard link */}
                        <Link href={userType === 'seller' ? '/dashboard/seller' : '/dashboard/customer'} className="block px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200 border-b border-gray-700">
                          Dashboard
                        </Link>
                        
                        {/* Wishlist link */}
                        <Link href="/wishlist" className="block px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200 border-b border-gray-700">
                          Wishlist
                        </Link>
                        
                        {/* Cart link */}
                        <Link href="/cart" className="block px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200 border-b border-gray-700">
                          Cart
                        </Link>
                        
                        {/* Orders link */}
                        <Link href="/orders" className="block px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200 border-b border-gray-700">
                          Orders
                        </Link>
                        ...
                        <button 
                          ...
                          className="w-full text-left px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200 text-red-400 hover:text-red-300"
                        >
                          Logout
                        </button>
                      </div>
              ...
              <div className="flex flex-col items-center">
                {isUserLoggedIn ? (
                  <div className="relative group">
                    <button 
                      type="button"
                      className="p-2 text-gray-300 hover:text-white transition-colors duration-300 cursor-pointer bg-transparent border-none"
                    >
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                      </svg>
                    ...
                    <div className="absolute right-0 top-full mt-2 w-56 bg-gradient-to-b from-gray-800 to-gray-900 text-white shadow-2xl rounded-none border border-gray-700 hidden group-hover:block z-50">
                      <div className="py-2">
                        ...
                        {/* Wishlist link */}
                        <Link href="/wishlist" className="block px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200 border-b border-gray-700 flex items-center">
                          <span>Wishlist</span>
                          {wishlistCount > 0 && (
                        ...
                        {/* Cart link */}
                        <Link href="/cart" className="block px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200 border-b border-gray-700 flex items-center">
                          <span>Cart</span>
                          {cartCount > 0 && (
                            <span className="ml-2 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                              {cartCount}
                  ...
                  <Link href="/login" className="p-2 text-gray-300 hover:text-white transition-colors duration-300">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                    </svg>
                  </Link>
...
function MobileMenu({ setIsMenuOpen }) {
  ...
  const loadCartAndWishlistCounts = async () => {
    ...
    try {
      ...
      console.error('Error loading cart and wishlist counts in MobileMenu:', error);
      // Reset counts on error
      setCartCount(0);
      setWishlistCount(0);
    }
        ...
        {isUserLoggedIn ? (
          <>
            ...
            <div className="px-4 py-3 border-b bg-gray-50">
              <p className="font-medium text-amber-600">Welcome, {getUserName()}!</p>
            </div>
            
            <Link 
              href={userType === 'seller' ? '/dashboard/seller' : '/dashboard/customer'} 
              className="block px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200"
              onClick={() => setIsMenuOpen(false)}
            >
              Dashboard
            </Link>
            <Link 
              href="/wishlist" 
              className="block px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200 flex items-center"
              onClick={() => {
            ...
            <button 
              ...
              className="w-full text-left px-4 py-3 hover:bg-amber-500 hover:bg-opacity-20 transition-colors duration-200 text-red-500"
            >
              Logout
            </button>
          </>
          ...
          <>
            ...
            <Link 
              ...
              className="block px-4 py-3 hover:bg-gray-100"
              onClick={() => {
                // Store the register page as intended destination
