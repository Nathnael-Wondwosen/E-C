...
const EcosystemRoutingTiles = dynamic(() => import('../components/EcosystemRoutingTiles'));
const MarketScopeSwitcher = dynamic(() => import('../components/MarketScopeSwitcher'));
const HotDealsSection = dynamic(() => import('../components/HotDealsSection'));
...
const TrendingProductsSection = dynamic(() => import('../components/TrendingProductsSection'));
const FullWidthBannerSection = dynamic(() => import('../components/FullWidthBannerSection'));
const RandomProductsSection = dynamic(() => import('../components/RandomProductsSection'));
const NewsBlogSection = dynamic(() => import('../components/NewsBlogSection'));
const TradexTVSection = dynamic(() => import('../components/TradexTVSection'));

const inferScopeFromProduct = (product, index) => {
  ...
  if (scope === 'local' || scope === 'africa' || scope === 'global' || scope === 'china' || scope === 'b2b') return scope;
  if (productType === 'b2b') return 'b2b';
...
export default function Home() {
  ...
  const scopeCounts = allProducts.reduce(
    (acc, product, index) => {
      const scope = inferScopeFromProduct(product, index);
      ...
      if (origin.includes('china') || `${product.marketScope || ''}`.toLowerCase() === 'china') {
        acc.china = (acc.china || 0) + 1;
      }
      return acc;
    },
    { local: 0, africa: 0, global: 0, china: 0, b2b: 0 }
  );
  const scopeProducts = useMemo(() => {
    const grouped = { local: [], africa: [], global: [], china: [], b2b: [] };
    allProducts.forEach((product, index) => {
      const scope = inferScopeFromProduct(product, index);
      if (!grouped[scope]) return;
      grouped[scope].push({
        id: product.id,
    ...
    return {
      local: grouped.local.slice(0, 12),
      africa: grouped.africa.slice(0, 12),
      global: grouped.global.slice(0, 12),
      china: grouped.china.slice(0, 12),
      b2b: grouped.b2b.slice(0, 12),
    };
  }, [allProducts]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        <title>TradeEthiopia Gateway | B2B + B2C Commerce Platform</title>
        <meta
          name="description"
          content="TradeEthiopia gateway homepage with hero search, ecosystem routing, directory menu, latest B2C items, Made in Ethiopia highlights, and global market switching."
        />
        <meta name="keywords" content="TradeEthiopia, B2B marketplace, B2C marketplace, Made in Ethiopia, Africa trade, global trade" />
        <meta property="og:title" content="TradeEthiopia Gateway | B2B + B2C Commerce Platform" />
        <meta
          property="og:description"
          content="Explore local, Africa, and global marketplace routes with modern, multilingual gateway navigation."
        />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://tradeethiopia.com/" />
        <meta property="og:image" content="/hero-background.jpg" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="TradeEthiopia Gateway" />
        <meta
          name="twitter:description"
          content="A responsive multi-market gateway for B2B/B2C discovery, routing, and conversion."
        />
        <link rel="canonical" href="https://tradeethiopia.com/" />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              '@type': 'WebSite',
              name: 'TradeEthiopia',
              url: 'https://tradeethiopia.com/',
              potentialAction: {
                '@type': 'SearchAction',
                target: 'https://tradeethiopia.com/marketplace?search={search_term_string}',
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-gray-400">
                <li><Link href="/localmarket" className="hover:text-white transition">B2B Products</Link></li>
                <li><Link href="/localmarket" className="hover:text-white transition">Wholesale Orders</Link></li>
                <li><Link href="/localmarket" className="hover:text-white transition">Supplier Verification</Link></li>
                <li><Link href="/localmarket" className="hover:text-white transition">Trade Shows</Link></li>
              </ul>
            </div>
          </div>
