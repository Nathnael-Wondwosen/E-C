...
export default function InquiryCenterPage () {
  ...
  const loadInquiries = useCallback(
    async ({ showLoader = false, silent = false } = {}) => {
      ...
      try {
        ...
        const sellerSession = getRequiredCustomerSession('seller');
        const buyerSession = getRequiredCustomerSession();
        const session = sellerSession.loggedIn ? sellerSession : buyerSession;
        const storedUserType = session.userType || 'buyer';
        const userId = String(session.userId || '').trim();

        if (!session.loggedIn || !userId) {
          router.push('/login');
          return;
        }
  ...
  return (
    <div className="portal-page min-h-screen">
      ...
      <main className="mx-auto h-[calc(100vh-56px)] max-w-[1450px] px-0 py-0 pb-[4.6rem] sm:px-4 sm:py-3 sm:pb-3 lg:px-6">
        {sortedRows.length === 0 ? (
          ...
          <section className="grid h-full min-h-0 overflow-hidden rounded-none border-0 bg-[#DDE6EC] shadow-none grid-cols-1 sm:mx-0 sm:rounded-none sm:border sm:border-[#BFD0DB] sm:shadow-[0_10px_32px_rgba(15,23,42,0.12)] lg:grid-cols-[300px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)]">
            ...
            {activeInquiry ? (
              <div className={`min-h-0 flex-col bg-[#D4DEE5] ${isMobileThreadOpen ? 'flex' : 'hidden lg:flex'}`}>
                    ...
                    <div className="flex min-w-0 items-center gap-2">
                      {getProductImageUrl(activeInquiry) ? (
                        <img
                          ...
                          className="h-9 w-11 shrink-0 rounded-sm border border-[#C7D4DF] object-cover sm:h-10 sm:w-12"
                        />
                      ) : (
                        <div className="flex h-9 w-11 shrink-0 items-center justify-center rounded-sm border border-[#C7D4DF] bg-[#E9F0F6] text-[11px] text-[#6E8497] sm:h-10 sm:w-12 sm:text-xs">
                          IMG
                        </div>
                      )}
                      <div className="min-w-0">
                    ...
                    <div className="flex w-full flex-wrap items-center justify-end gap-1.5 sm:w-auto sm:gap-2">
                      {activeContactDetails.phone ? (
                        <a
                          ...
                          className="inline-flex h-8 items-center gap-1.5 rounded-xl border border-emerald-300 bg-emerald-50 px-2.5 py-1 text-[12px] font-semibold text-emerald-700 sm:h-9 sm:gap-2 sm:px-3 sm:text-sm"
                        >
                        ...
                        <span className="inline-flex h-8 items-center rounded-xl border border-slate-300 bg-slate-50 px-2.5 py-1 text-[12px] font-semibold text-slate-500 sm:h-9 sm:px-3 sm:text-sm">
                          Un verified
                        </span>
                      )}
                      {userType === 'seller' ? (
                        <>
                          <button
                            ...
                            className="rounded-lg border border-amber-300 bg-amber-50 px-2.5 py-1 text-[10px] font-semibold text-amber-700 disabled:opacity-60 sm:text-[11px]"
                          >
                            Contacted
                          </button>
                          <button
                            ...
                            className="rounded-lg border border-emerald-300 bg-emerald-50 px-2.5 py-1 text-[10px] font-semibold text-emerald-700 disabled:opacity-60 sm:text-[11px]"
                          >
                            Closed
                          </button>
                        </>
                ...
                <div ref={chatScrollRef} className="scrollbar-hidden min-h-0 flex-1 overflow-y-auto bg-[#CFD9E1] px-2.5 py-2.5 sm:px-4 sm:py-3">
                  <div className="mx-auto mb-2.5 w-fit rounded-full border border-[#F0A552] bg-[#FFF4E7] px-4 py-1 text-[12px] text-[#926022] sm:mb-3 sm:px-6 sm:py-1.5 sm:text-sm">
                      ...
                      return (
                        <div key={item?.id || `${activeInquiryId}-${item?.createdAt || ''}-${item?.text || ''}`}>
                          ...
                          <div className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[82%] rounded-2xl px-3 py-2 sm:max-w-[78%] sm:px-4 ${
                              isMine
                                ? 'rounded-br-md bg-[#B8DBBF] text-[#19324A]'
                                : 'rounded-bl-md bg-white text-[#19324A]'
                            }`}>
                              <p className="whitespace-pre-line text-[13px] leading-5 sm:text-[15px] sm:leading-6">{item?.text || '-'}</p>
                ...
                <div className="border-t border-[#D7E0EA] bg-[linear-gradient(180deg,#F8FAFC_0%,#F4F7FB_100%)] px-2.5 py-1.5 sm:px-4 sm:py-2.5">
                  {userType !== 'seller' ? (
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-white text-[#6A8BA3] shadow-[0_8px_18px_rgba(15,23,42,0.06)] sm:h-9 sm:w-9">
                    ...
                    <input
                      ...
                      className="h-9 flex-1 rounded-[1rem] border border-[#CAD5E2] bg-white px-3 text-[13px] text-[#314B62] placeholder:text-[#7F97AB] shadow-[0_8px_18px_rgba(15,23,42,0.04)] sm:h-11 sm:rounded-[1.1rem] sm:px-4 sm:text-base"
                      onKeyDown={(event) => {
                    ...
                    <button
                      ...
                      className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[linear-gradient(135deg,#34D399,#10B981)] text-white shadow-[0_14px_24px_rgba(16,185,129,0.28)] disabled:opacity-60 sm:h-11 sm:w-11"
                    >
                      {replyActionState[activeInquiryId] ? (
                        <span className="text-xs">...</span>
                      ) : (
                        <svg className="h-5 w-5 sm:h-5.5 sm:w-5.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {userType === 'seller' ? (
        <nav
          className="fixed inset-x-0 bottom-0 z-40 border-t border-[#D8E1EA] bg-white/95 shadow-[0_-10px_35px_rgba(15,23,32,0.08)] backdrop-blur md:hidden"
          style={{ paddingBottom: 'max(0px, env(safe-area-inset-bottom))' }}
        >
          <div className="mx-auto grid max-w-md grid-cols-5">
            <SellerMessagesNavItem href="/dashboard/seller" label="Home">
            ...
            <SellerMessagesNavItem href="/inquiries" label="Messages" active>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" className="h-5 w-5">
            ...
            <SellerMessagesNavItem href="/dashboard/seller/products" label="Store">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" className="h-5 w-5">
            ...
            <SellerMessagesNavItem href="/profile" label="Account">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" className="h-5 w-5">
