...
export default function Products() {
  ...
  useEffect(() => {
    const fetchFirstPage = async () => {
      try {
        setLoading(true);
        const result = await getProductsPage(1, 24);
        const fetchedProducts = result.items || [];
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const result = await getProductsPage(nextPage, 24);
      const fetchedProducts = result.items || [];
      const regularProducts = fetchedProducts.filter(product => product.productType !== 'B2B');
      const convertedProducts = regularProducts.map(product => ({
        ...product,
      ...
      setProducts((prev) => {
        ...
        const deduped = convertedProducts.filter((p) => !existingIds.has(String(p.id)));
        return [...prev, ...deduped];
      });
      setPage(nextPage);
          ...
          <>
            ...
            {filteredProducts.length === 0 ? (
              ...
              <>
                ...
                {filter.trim() === '' && page < totalPages && (
                  <div className="mt-8 flex justify-center">
                    <button
                      onClick={loadMore}
                      disabled={loadingMore}
                      className="rounded-md bg-gray-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-black disabled:opacity-60"
                    >
